...
require_once("../datas/sql.php");//ユーザー情報テーブル
require_once("./getcharacterlist.php");//キャラクターデータ取得メゾット
...
//マスターデータファイル
 $characterfilepath="../datas\csv\CharactersStetas.csv";

 function setStetas($characterId){
    ...
    if($characterId !== null){
        ...
        $returnstetas = array(
            ...
            "stetas"=>array(
                "hp"=>$character_record[15],
                "pow"=>$character_record[16],
                "def"=>$character_record[17],
                "magic"=>$character_record[18],
                "mental"=>$character_record[19]
            )
        );
    }else{
        //0なら空データを返す
...
function setpartyIds($userid_){//MySQL間のデータ更新と取得
    ...
    foreach($party_member_ids as $member){
        //キャラレコード取得,格納
        array_push($party_stetas,  setStetas($member));
    }
...
try{
    ...
    if(!isset($_SESSION["PARTY_STETAS"])){
        ...
        if( $nullparty === 0 || $select_characterId){
            //編成するキャラクター情報取得
            $selectdata = setStetas($select_characterId);//表示するキャラレコード更新
